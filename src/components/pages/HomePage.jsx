...
function formatDateMonthYear(dateStr, isEnglish = false) {
...
export default function HomePage() {
  ...
  const projectsToShow = messages?.projects?.pinProjects?.slice(0, 4) || [];
  const basePath = isEnglish ? "/en/projects" : "/projects";
  const experience = messages?.experience || [];
  const education = messages?.education || [];
  const email = messages?.profile?.email || "palmiergianluca@gmail.com";
  const about = messages?.profile?.about || "";
  ...
  return (
    <>
      ...
      >
        ...
        >
          <h3 className="mb-6 text-lg font-medium">
            {messages?.sections?.education || "Educación"}
          </h3>
          <div className="flex flex-col gap-8">
            {education.map((item) => (
